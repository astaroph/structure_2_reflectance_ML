...
class PairedImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return sem_image, reflectance_values
...
# Define your data transformation for testing (if different from training)
...
def test_accuracy_perchannel(test_loader, model):
    ...
    with torch.no_grad():
        for i, data in enumerate(test_loader, 0):
            structural_inputs, reflectance_targets = data[0].to(device), data[1].to(device)

            # Forward pass
            reflectance_predictions = model(structural_inputs)
...
def train_and_evaluate_model(lr, batch_size, num_epochs=10,decay=0.00001):
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(paired_dataloader, 0):
            ...
            # Forward pass
            reflectance_predictions = regression(augmented_structural_inputs)
